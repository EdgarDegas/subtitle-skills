...
Manual cue retries allow at most three Iris attempts, including model-call failures reported as workflow errors, malformed response JSON, invalid candidate IDs, and cue-validation failures. Log each failed attempt and pass its error into the next prompt. If all attempts fail, report the last error and leave existing saved corrections intact. User interruptions and local I/O errors stop immediately.
